...
; Variables for adjusting the effect
	Global GrayscaleAlpha# = 1.0
	Global GrayscaleBrightness# = 0.8
	Global GrayscaleMethod = 0
	Global GrayscaleAlphaTexture = 0
...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 And GrayscaleAlpha<1 Then GrayscaleAlpha = GrayscaleAlpha + 0.1
		If KeyHit(3)>0 And GrayscaleAlpha>0 Then GrayscaleAlpha = GrayscaleAlpha - 0.1
		If KeyHit(4)>0 And GrayscaleBrightness<1 Then GrayscaleBrightness = GrayscaleBrightness + 0.1
		If KeyHit(5)>0 And GrayscaleBrightness>0 Then GrayscaleBrightness = GrayscaleBrightness - 0.1		
		If KeyHit(6) Then GrayscaleMethod = 1 - GrayscaleMethod
		If KeyHit(7) Then GrayscaleAlphaTexture = AlphaTexture - GrayscaleAlphaTexture
	
	
	
	; Customize effect
	CustomPostprocessGrayscale GrayscaleAlpha, GrayscaleBrightness, GrayscaleMethod, GrayscaleAlphaTexture
	
	
	RenderPostprocess FE_Grayscale		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,70,"keys 1/2 - increase/decrease Grayscale alpha value: "+Str(GrayscaleAlpha)
		Text 10,90,"key 3/4 - increase/decrease Grayscale brightness: "+Str(GrayscaleBrightness)
		Text 10,110,"key 5 - enable/disable Grayscale method (inverse): "+Str(GrayscaleMethod)
		Text 10,130,"key 6 - enable/disable Grayscale alpha-texture: $"+Hex(GrayscaleAlphaTexture)
	EndIf
End Function