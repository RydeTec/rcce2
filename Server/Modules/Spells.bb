...
Function Regenerate(A.ActorInstance) 
    If ((A\Attributes\Value[HealthStat] < A\Attributes\Maximum[HealthStat]) And (A\Attributes\Value[HealthStat] > 0)) 
       Local HealBy% = (A\Attributes\Maximum[HealthStat] * RegenAmountValue#) + A\Attributes\Value[HealthStat] 
       If (HealBy% < A\Attributes\Maximum[HealthStat]) 
          A\Attributes\Value[HealthStat] = HealBy% 
       Else 
          A\Attributes\Value[HealthStat] = A\Attributes\Maximum[HealthStat] 
       EndIf 
        
       Pa$ = "A" + RCE_StrFromInt$(A\RuntimeID, 2) + RCE_StrFromInt$(HealthStat, 1) + RCE_StrFromInt$(A\Attributes\Value[HealthStat], 2) 
       AInstance.AreaInstance = Object.AreaInstance(A\ServerArea) 
       A1.ActorInstance = AInstance\FirstInZone 
       While A1 <> Null 
          If A1\RNID > 0 Then RCE_Send(Host, A1\RNID, P_StatUpdate, Pa$, True) 
          A1 = A1\NextInZone 
       Wend 
    EndIf 
 End Function