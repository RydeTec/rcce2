...
.\compile.bat -r           # also build the Rust apps (client + server) → bin\ClientRS.exe + bin\ServerRS.exe (needs cargo; skips if absent)
.\compile.bat -e -t -r     # build ONLY the Rust apps (skip engine + tools)
...
**Rust apps (`bin\ClientRS.exe` + `bin\ServerRS.exe`).** `compile.bat -r` / `compile.sh -r` (`-r` = "rust", build **all** Rust apps) `cargo build --release`s the `client-rs` `client-window` binary → `bin\ClientRS.exe` **and** the `server-rs` `rcce-server` binary → `bin\ServerRS.exe` (the headless authoritative server; Linux is its real deploy target — see `server-rs/README.md`). Both are self-contained drop-ins alongside the Blitz `bin\Client.exe` / `bin\Server.exe` (they coexist; `-r` never touches the Blitz binaries). The flag is opt-in (a clean wgpu build is ~1–2 min) and skips gracefully with a message if `cargo` isn't on `PATH`, so CI without Rust is unaffected. `ClientRS.exe` finds the project `data\` via `RCCE_DATA`, else a `data\` dir next to or above its own location (so `bin\..\data` resolves) or the working directory. It boots into a **login screen** (account/password) → **character select** (create / delete / enter world), then connects to a running `bin\Server.exe -UNLOCK` and renders the live world. See `client-rs/` and the **Rust client port** memory for the feature set.
