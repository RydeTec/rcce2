...
└── Modules/
    ...
    └── Loom/
        ...
        │                            Emitters_Rebuild resyncs it after a
        │                            Particles-tab create / delete / save
        ├── ParticleEditor.bb        boot-loads the full RP_EmitterConfig
        │                            objects (field data) for the Particles
        │                            browser tab; save-all / free-all /
        │                            unique-name helpers (RP_* writers)
...
- **`Browser.bb`** — The boot surface with 18 categories: Actors / Items / Spells / Projectiles / Particles / Zones / Factions / Animation Sets / Tools / Scripts / Textures / Meshes / Sounds / Music / Stats / Days & Seasons / Interface / Settings. Content and asset categories dispatch to per-kind card grids; Tools uses `drawToolsGrid` to launch companion editors, while Stats, Days & Seasons, Interface, and Settings are project-level surfaces. Filter input live-filters card grids; arrow keys move a brass-ringed selection cursor; Enter focuses. Zones has a `Card | Atlas` toggle that swaps the grid for the `Atlas` surface.
...
| `particle` | `Handle(RP_EmitterConfig)` (round-trips via `Object.RP_EmitterConfig(handle)`) | **no** — regenerates per load, like zones |
