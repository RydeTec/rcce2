...
| Current evidence | RED captured the absent observation projection, exact script diagnostic path, and overlay navigation state. Focused GREEN currently passes projection 15/0, GUI 36/0, and native smoke contract 1/0. Coverage includes evidence boundaries, deterministic domain order, exact accepted path routing, alias-quarantine non-invention, 257-row uncapped projection, fixed-height rows, stale routing, atomic replacement, Find/Return priority, and smoke fields. Full local gates, independent specification/quality review, native Windows smoke/visual evidence, integration, push, and exact-head CI remain pending |
...
| Remaining acceptance | Exact local gates; fresh specification review and different fresh quality review; native Windows valid/missing-root smoke plus interaction evidence where available; integration, push, exact-head CI; ledger and durable-state closeout |
...
1. Finish `SE-FB-MVP-10` exact local gates, independent review, native Windows feedback proof, integration, exact-head CI, and durable-state closeout without widening it into a complete validator or health verdict.
2. Preserve FB09's bounded raw-focus trail; do not silently widen it into formal history, recents, timeline, accessibility, or production-shell credit.
