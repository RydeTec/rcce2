...
| `"O"` | All other players in the same area | `1B sub + 2B Attacker\RuntimeID + 2B Victim\RuntimeID` (no damage payload) | Observer swing animation only. ClientNet animates/orients the attacker but does not read `Damage`, mutate the victim's HP, or create target-hit effects from this frame; observers receive HP separately through `P_StatUpdate`. |
