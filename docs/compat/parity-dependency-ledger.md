...
|---|---|---|---|---|
| `PD-M1-C-DATA` | `rcce-data` public read APIs for only the diagnostic domains, including sentinel, malformed, non-UTF8, raw-string, and topology behavior | **Blocked:** parsers exist, but the [format matrix](project-format-matrix.md) records semantic-loss and I1-only rows | Shared format/data; named owner unassigned | `SE-M1-P04/P05` accept byte/semantic consensus fixtures; disagreement stays provisional and disabled for authoritative diagnostics |
| `PD-M1-S-DATA` | Server semantic view of the same selected domains (`rcce-server-core` and direct `rcce-data` consumers) | **Blocked:** no accepted three-consumer fixture revision | Server parity + shared format/data; named owner unassigned | Same fixture bytes produce an explicitly compared server semantic view; differences become dependency rows, not editor rules |
