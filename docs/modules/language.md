...
- **`LoadLanguage(Filename$)`** — open `Filename$`, parse line-by-line (skipping `;` comments, stripping trailing comments, ignoring blanks), assign to `LanguageString$(0..MaxLanguageString)`. Upper-cases slash-command range. Returns `True` on success and `False` if the file cannot be opened or has more than `MaxLanguageString + 1` non-blank, non-comment lines; the overflow path closes the stream before returning. Bumping `MaxLanguageString` is mandatory when adding entries.
