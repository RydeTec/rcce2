...
The catalog above lists each packet's purpose. For the wire-encoding primitives, handler-side conventions, and per-packet field layouts, see the [wire-protocol reference](../protocol/index.md):

*   [`docs/protocol/index.md`](../protocol/index.md) — generated table of all 56 packets with handler line refs and direction (C→S / S→C / Both / Unused). Regenerated by `scripts/gen_packet_index.sh`.
*   [`docs/protocol/encoding.md`](../protocol/encoding.md) — `RCE_StrFromInt$` / `RCE_IntFromStr` / `ClampWorldCoord#` / length-prefixed strings / IEEE float round-trip.
*   [`docs/protocol/handler-conventions.md`](../protocol/handler-conventions.md) — soft-fail discipline, bounds-then-deref, handle-Null discipline, iterator-during-iteration, sibling-protection asymmetry, privilege gating.
*   [`docs/protocol/packets/`](../protocol/packets/) — per-packet detail pages (incrementally filled).