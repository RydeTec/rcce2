...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 And BlurAlpha<1 Then BlurAlpha = BlurAlpha + 0.1
		If KeyHit(3)>0 And BlurAlpha>0 Then BlurAlpha = BlurAlpha - 0.1
		If KeyHit(4)>0 And BlurPasses<16 Then BlurPasses = BlurPasses + 1
		If KeyHit(5)>0 And BlurPasses>1 Then BlurPasses = BlurPasses - 1
		If KeyHit(6)>0 And BlurRadius<10.0 Then BlurRadius = BlurRadius + 0.01
		If KeyHit(7)>0 And BlurRadius>0.01 Then BlurRadius = BlurRadius - 0.01
		If KeyHit(8) Then BlurQuality = (1 + BlurQuality) And 3
		If KeyHit(9) Then
	...
	; Customize effect
	CustomPostprocessBlur BlurAlpha, BlurPasses, BlurRadius, BlurQuality, BlurColorRed, BlurColorGreen, BlurColorBlue, BlurAlphaTexture
	
	
	RenderPostprocess FE_BLUR		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
