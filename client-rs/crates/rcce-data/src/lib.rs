...
pub use area::{
    AreaEnv, AreaScenery, EmitterPlacement, SceneryPlacement, TerrainPatch, WaterPlane,
};
pub use attributes::{AttributeDef, AttributeNames};
pub use b3d::{B3dAnim, B3dBone, B3dKey, B3dMesh, B3dModel};
pub use catalog::{
    MeshCatalog, MeshEntry, MeshParseEvidence, MeshRecordEvidence, MeshSlotDisposition,
    MusicCatalog, MusicEntry, ParsedCatalog, SoundCatalog, SoundEntry, TextureCatalog,
    TextureEntry, CATALOG_SLOTS,
};
...
mod tests {
    ...
    fn parse_real_interface_dat() {
        ...
        for a in &l.attributes {
            assert!(
                (0.0..=1.0).contains(&a.x) && (0.0..=1.0).contains(&a.y),
                "attr off-screen: {a:?}"
            );
        }
    }
    ...
    fn parse_real_attributes_dat() {
        ...
        eprintln!(
            "Attributes.dat ({} bytes), assignment {}:",
            bytes.len(),
            a.assignment
        );
    ...
    fn parse_real_b3d_models() {
        ...
        for rel in candidates {
            ...
            parsed_any = true;
            let model =
                B3dModel::parse(&bytes).unwrap_or_else(|e| panic!("{rel}: parse failed: {e}"));
            ...
            eprintln!(
                "{rel}: {} meshes, {vtx} verts, {tris} tris",
                model.meshes.len()
            );
        }
        if !parsed_any {
    ...
    fn parse_real_emitter_configs() {
        ...
        for name in [
            ...
            assert!(
                c.max_particles > 0 && c.max_particles < 1_000_000,
                "{name}: max {}",
                c.max_particles
            );
            assert!(
                c.lifespan > 0 && c.lifespan < 1_000_000,
                "{name}: life {}",
                c.lifespan
            );
            assert!((1..=3).contains(&c.shape), "{name}: shape {}", c.shape);
            assert!(
                (1..=3).contains(&c.blend_mode),
                "{name}: blend {}",
                c.blend_mode
            );
            assert!(
                c.scale_start.is_finite() && c.alpha_start.is_finite(),
                "{name}: non-finite"
            );
            eprintln!(
    ...
    fn parse_real_area_scenery() {
        ...
        for zone in ["Plains", "Test Zone", "Northern Shrine"] {
            ...
            any = true;
            let area = area::AreaScenery::parse(&bytes).unwrap_or_else(|e| panic!("{zone}: {e}"));
            assert!(
            ...
            for s in &area.sceneries {
                assert!(
                    s.pos.iter().all(|c| c.is_finite()),
                    "{zone}: non-finite pos"
                );
                assert!(
                    s.scale.iter().all(|c| c.is_finite()),
                    "{zone}: non-finite scale"
                );
            ...
            // (N+1)².
            assert!(
                area.terrains.len() < 4096,
                "{zone}: implausible terrain count {}",
                area.terrains.len()
            );
            for t in &area.terrains {
                assert_eq!(
                    t.heights.len(),
                    (t.grid as usize + 1).pow(2),
                    "{zone}: terrain grid mismatch"
                );
                assert!(
                    t.heights.iter().all(|h| h.is_finite()),
                    "{zone}: non-finite terrain height"
                );
                assert!(
                    t.pos.iter().all(|c| c.is_finite()) && t.rot.iter().all(|c| c.is_finite()),
                    "{zone}: non-finite terrain xform"
                );
            }
            eprintln!(
                "{zone}: {} water plane(s), {} LOD terrain(s)",
                area.waters.len(),
                area.terrains.len()
            );
            ...
            eprintln!(
                "{zone}: sky_tex_id={sky} -> {sky_file:?}  fog={:?}",
                area.env.fog_color
            );
            ...
            eprintln!(
                "{zone}: cloud={} -> {:?}  stars={} -> {:?}",
                area.env.cloud_tex_id,
                resolve(area.env.cloud_tex_id),
                area.env.stars_tex_id,
                resolve(area.env.stars_tex_id)
            );
    ...
    fn skinning_bind_is_identity_and_frame_deforms() {
        ...
        assert!(
            max_err < 1e-2,
            "bind-pose skin drifted from bind geometry: {max_err}"
        );
        ...
        assert!(
            moved > 100,
            "frame 2 barely deformed the mesh ({moved} verts moved)"
        );
