...
Function HelpGuardPrecedesReleaseCleanup%(Path$)
	...
	While Not Eof(F)
		Line$ = ReadLine$(F)
		If Stage = 0 And Instr(Line$, "set " + Chr$(34) + "HELP_ARG=" + Chr$(34)) > 0 Then Stage = 1
		If Stage = 1 And Instr(Line$, "for %%A in (%*) do (") > 0 Then Stage = 2
		If Stage = 2 And Instr(Line$, "if " + Chr$(34) + "%%~A" + Chr$(34) + "==" + Chr$(34) + "-h" + Chr$(34) + " set " + Chr$(34) + "HELP_ARG=%%~A" + Chr$(34)) > 0 Then Stage = 3
		If Stage = 3 And Instr(Line$, "if " + Chr$(34) + "%%~A" + Chr$(34) + "==" + Chr$(34) + "--help" + Chr$(34) + " set " + Chr$(34) + "HELP_ARG=%%~A" + Chr$(34)) > 0 Then Stage = 4
		If Stage = 4 And Instr(Line$, "if defined HELP_ARG goto help") > 0 Then Stage = 5
		If Stage = 5 And Instr(Line$, "goto publish") > 0 Then Stage = 6
		If Stage = 6 And Instr(Line$, ":help") > 0 Then Stage = 7
		If Stage = 7 And Instr(Line$, "call " + Chr$(34) + "%ROOTDIR%\\compile.bat" + Chr$(34) + " %HELP_ARG%") > 0 Then Stage = 8
		If Stage = 8 And Instr(Line$, "set " + Chr$(34) + "HELP_STATUS=%ERRORLEVEL%" + Chr$(34)) > 0 Then Stage = 9
		If Stage = 9 And Instr(Line$, "endlocal & exit /b %HELP_STATUS%") > 0 Then Stage = 10
		If Stage = 10 And Instr(Line$, "if exist " + Chr$(34) + "%ROOTDIR%\\release" + Chr$(34) + " rmdir /S /Q ") > 0
			CloseFile F
			Return True
		EndIf
	Wend
